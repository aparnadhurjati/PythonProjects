...
class CodeReview():
    ...
    @agent
    def syntax_checker(self) -> Agent:
        return Agent(
            config=self.agents_config['syntax_checker'], # type: ignore[index]
            verbose=True
        )
    ...
    @task
    def syntax_check_task(self) -> Task:
        return Task(
            config=self.tasks_config['syntax_check_task'], # type: ignore[index]
        )

    @task
    def design_review_task(self) -> Task:
        return Task(
            config=self.tasks_config['design_review_task'], # type: ignore[index]
        )
